...
) -> Result<mys_sdk_types::ZkLoginPublicIdentifier, SignatureError> {
    const ISS: &str = "iss";

    let iss = verify_extended_claim(&inputs.iss_base64_details, ISS)?;
    mys_sdk_types::ZkLoginPublicIdentifier::new(iss, inputs.address_seed.clone())
