...
use mso_sdk_types::Claim;
use mso_sdk_types::Jwk;
use mso_sdk_types::JwkId;
use mso_sdk_types::UserSignature;
use mso_sdk_types::ZkLoginAuthenticator;
...
) -> Result<mso_sdk_types::ZkLoginPublicIdentifier, SignatureError> {
    const ISS: &str = "iss";

    let iss = verify_extended_claim(&inputs.iss_base64_details, ISS)?;
    mso_sdk_types::ZkLoginPublicIdentifier::new(iss, inputs.address_seed.clone())
