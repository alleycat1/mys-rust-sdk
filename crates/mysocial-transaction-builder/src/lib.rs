...
use error::Error;
use mysocial_types::Address;
use mysocial_types::Argument;
use mysocial_types::Command;
use mysocial_types::GasPayment;
use mysocial_types::Identifier;
use mysocial_types::Input;
use mysocial_types::MakeMoveVector;
use mysocial_types::MergeCoins;
use mysocial_types::MoveCall;
use mysocial_types::ObjectId;
use mysocial_types::ObjectReference;
use mysocial_types::Publish;
use mysocial_types::SplitCoins;
use mysocial_types::Transaction;
use mysocial_types::TransactionExpiration;
use mysocial_types::TransferObjects;
use mysocial_types::TypeTag;
use mysocial_types::Upgrade;
...
impl TransactionBuilder {
    ...
    ///  ```rust,ignore
    ///  use mysocial_graphql_client::Client;
    ///  use mysocial_sdk_types::unresolved;
    ///  use mysocial_transaction_builder::TransactionBuilder;
    ///  use mysocial_transaction_builder::Function;
    ///
    ...
    pub fn finish(self) -> Result<Transaction, Error> {
        ...
        Ok(Transaction {
            kind: mysocial_types::TransactionKind::ProgrammableTransaction(
                mysocial_types::ProgrammableTransaction {
                    inputs: self
                        .inputs
                        .into_iter()
...
mod tests {
    ...
    use mysocial_crypto::ed25519::Ed25519PrivateKey;
    use mysocial_crypto::MySocialSigner;
    use mysocial_graphql_client::faucet::CoinInfo;
    use mysocial_graphql_client::faucet::FaucetClient;
    use mysocial_graphql_client::Client;
    use mysocial_graphql_client::PaginationFilter;
    use mysocial_types::Address;
    use mysocial_types::ExecutionStatus;
    use mysocial_types::IdOperation;
    use mysocial_types::ObjectId;
    use mysocial_types::ObjectType;
    use mysocial_types::TransactionEffects;
    use mysocial_types::TypeTag;
    ...
    use mysocial_types::TransactionDigest;

    /// Type corresponding to the output of `mysocial move build --dump-bytecode-as-base64`
    ...
    /// This is used to read the json file that contains the modules/deps/digest generated with mysocial
    ...
    /// root of the mysocial-transaction-builder crate.
    ...
    async fn wait_for_tx_and_check_effects_status_success(
        ...
        effects: Result<Option<TransactionEffects>, mysocial_graphql_client::error::Error>,
    ) {
                ...
                TransactionEffects::V2(e) => {
                    for obj in e.changed_objects.clone() {
                        if obj.id_operation == IdOperation::Created {
                            ...
                            match change {
                                mysocial_types::ObjectOut::PackageWrite { .. } => {
                                    package_id = Some(obj.object_id);
                                }
                                mysocial_types::ObjectOut::ObjectWrite { .. } => {
            ...
            match obj.object_type() {
                ObjectType::Struct(x) if x.name.to_string() == "UpgradeCap" => {
                    match obj.owner() {
                        ...
                        mysocial_types::Owner::Object(_) => {
                            panic!("Upgrade capability controlled by object")
                        }
                        mysocial_types::Owner::Immutable => panic!("Upgrade capability is stored immutably and cannot be used for upgrades"),
                    };
                    break;
                }
                _ => {}
