...
use mso_types::Argument;
use mso_types::Command;
use mso_types::GasPayment;
use mso_types::Identifier;
use mso_types::Input;
use mso_types::MakeMoveVector;
use mso_types::MergeCoins;
use mso_types::MoveCall;
use mso_types::ObjectId;
use mso_types::ObjectReference;
use mso_types::Publish;
use mso_types::SplitCoins;
use mso_types::Transaction;
use mso_types::TransactionExpiration;
use mso_types::TransferObjects;
use mso_types::TypeTag;
use mso_types::Upgrade;
...
impl TransactionBuilder {
    ...
    ///  use mso_sdk_types::unresolved;
    ///  use mso_transaction_builder::TransactionBuilder;
    ///  use mso_transaction_builder::Function;
    ///
    ...
    pub fn finish(self) -> Result<Transaction, Error> {
        ...
        Ok(Transaction {
            kind: mso_types::TransactionKind::ProgrammableTransaction(
                mso_types::ProgrammableTransaction {
                    inputs: self
                        .inputs
                        .into_iter()
...
mod tests {
    ...
    use mso_crypto::ed25519::Ed25519PrivateKey;
    use mso_crypto::MySocialSigner;
    use mso_graphql_client::faucet::CoinInfo;
    use mso_graphql_client::faucet::FaucetClient;
    use mso_graphql_client::Client;
    use mso_graphql_client::PaginationFilter;
    use mso_types::Address;
    use mso_types::ExecutionStatus;
    use mso_types::IdOperation;
    use mso_types::ObjectId;
    use mso_types::ObjectType;
    use mso_types::TransactionEffects;
    use mso_types::TypeTag;
    ...
    use mso_types::TransactionDigest;

    /// Type corresponding to the output of `mso move build --dump-bytecode-as-base64`
    ...
    /// This is used to read the json file that contains the modules/deps/digest generated with mso
    ...
    /// root of the mso-transaction-builder crate.
    ...
    async fn wait_for_tx_and_check_effects_status_success(
        ...
        effects: Result<Option<TransactionEffects>, mso_graphql_client::error::Error>,
    ) {
                ...
                TransactionEffects::V2(e) => {
                    for obj in e.changed_objects.clone() {
                        if obj.id_operation == IdOperation::Created {
                            ...
                            match change {
                                mso_types::ObjectOut::PackageWrite { .. } => {
                                    package_id = Some(obj.object_id);
                                }
                                mso_types::ObjectOut::ObjectWrite { .. } => {
            ...
            match obj.object_type() {
                ObjectType::Struct(x) if x.name.to_string() == "UpgradeCap" => {
                    match obj.owner() {
                        ...
                        mso_types::Owner::Object(_) => {
                            panic!("Upgrade capability controlled by object")
                        }
                        mso_types::Owner::Immutable => panic!("Upgrade capability is stored immutably and cannot be used for upgrades"),
                    };
                    break;
                }
                _ => {}
