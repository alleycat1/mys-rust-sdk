...
use mysocial_types::framework::Coin;
use mysocial_types::Address;
use mysocial_types::CheckpointDigest;
use mysocial_types::CheckpointSequenceNumber;
use mysocial_types::CheckpointSummary;
use mysocial_types::Event;
use mysocial_types::MovePackage;
use mysocial_types::Object;
use mysocial_types::SignedTransaction;
use mysocial_types::Transaction;
use mysocial_types::TransactionDigest;
use mysocial_types::TransactionEffects;
use mysocial_types::TransactionKind;
use mysocial_types::TypeTag;
use mysocial_types::UserSignature;
...
const MAINNET_HOST: &str = "https://mysocial-mainnet.mystenlabs.com/graphql";
const TESTNET_HOST: &str = "https://mysocial-testnet.mystenlabs.com/graphql";
const DEVNET_HOST: &str = "https://mysocial-devnet.mystenlabs.com/graphql";
...
impl Client {
    ...
    /// Coin type will default to `0x2::coin::Coin<0x2::mysocial::MySo>` if not provided.
    ...
    /// coins. For MySo coin, pass in the coin type: `0x2::coin::Coin<0x2::mysocial::MySo>`.
    ...
    /// ```rust,ignore
    ///
    /// let client = mysocial_graphql_client::Client::new_devnet();
    ...
    pub async fn object(&self, address: Address, version: Option<u64>) -> Result<Option<Object>> {
        ...
        if let Some(object) = response.data {
            ...
            let object = bcs
                .map(|b| bcs::from_bytes::<mysocial_types::Object>(&b))
                .transpose()?;
    ...
    pub async fn resolve_mysocialns_to_address(&self, domain: &str) -> Result<Option<Address>> {
        ...
        Ok(response
            .data
            .and_then(|d| d.resolve_mysocialns_address)
            .map(|a| a.address))
    }

    /// Get the default MySocialns domain name for the provided address.
    pub async fn default_mysocialns_name(&self, address: Address) -> Result<Option<String>> {
        ...
        Ok(response
            ...
            .and_then(|a| a.default_mysocialns_name))
    }
}
...
mod tests {
    ...
    use mysocial_types::Ed25519PublicKey;
    use mysocial_types::TypeTag;
    ...
    async fn test_coin_metadata_query() {
        let client = test_client();
        let cm = client.coin_metadata("0x2::mysocial::MySo").await;
        assert!(
            cm.is_ok(),
    ...
    async fn test_total_supply() {
        let client = test_client();
        let ts = client.total_supply("0x2::mysocial::MySo").await;
        assert!(
            ts.is_ok(),
    ...
    async fn test_package_by_name() {
        let client = Client::new_testnet();
        let package = client.package_by_name("mysocial@mysocial").await;
