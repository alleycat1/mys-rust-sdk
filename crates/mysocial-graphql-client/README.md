...
Note that this [`FaucetClient`] is explicitly designed to work with two endpoints: `v1/gas`, and `v1/status`. When passing in the custom faucet URL, skip the final endpoint and only pass in the top-level url (e.g., `https://faucet.devnet.mysocial.io`).
```rust, no_run
use mysocial_graphql_client::faucet::FaucetClient;
use mysocial_types::Address;
...
When using `cynic` and `mysocial-graphql-client`, you will need to register the schema by calling `mysocial-graphql-client-build::register_schema` in a `build.rs` file. See [mysocial-graphql-client-build](https://github.com/MystenLabs/mysocial-rust-sdk/tree/master/crates/mysocial-graphql-client-build) for more information.
