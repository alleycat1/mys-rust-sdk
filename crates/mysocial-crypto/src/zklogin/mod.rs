...
use mysocial_sdk_types::Claim;
use mysocial_sdk_types::Jwk;
use mysocial_sdk_types::JwkId;
use mysocial_sdk_types::UserSignature;
use mysocial_sdk_types::ZkLoginAuthenticator;
...
) -> Result<mysocial_sdk_types::ZkLoginPublicIdentifier, SignatureError> {
    const ISS: &str = "iss";

    let iss = verify_extended_claim(&inputs.iss_base64_details, ISS)?;
    mysocial_sdk_types::ZkLoginPublicIdentifier::new(iss, inputs.address_seed.clone())
