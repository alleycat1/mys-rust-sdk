...
use mso_types::framework::Coin;
use mso_types::Address;
use mso_types::CheckpointDigest;
use mso_types::CheckpointSequenceNumber;
use mso_types::CheckpointSummary;
use mso_types::Event;
use mso_types::MovePackage;
use mso_types::Object;
use mso_types::SignedTransaction;
use mso_types::Transaction;
use mso_types::TransactionDigest;
use mso_types::TransactionEffects;
use mso_types::TransactionKind;
use mso_types::TypeTag;
use mso_types::UserSignature;
...
const MAINNET_HOST: &str = "https://mso-mainnet.mystenlabs.com/graphql";
const TESTNET_HOST: &str = "https://mso-testnet.mystenlabs.com/graphql";
const DEVNET_HOST: &str = "https://mso-devnet.mystenlabs.com/graphql";
...
impl Client {
    ...
    /// Coin type will default to `0x2::coin::Coin<0x2::mso::MSO>` if not provided.
    ...
    /// coins. For MSO coin, pass in the coin type: `0x2::coin::Coin<0x2::mso::MSO>`.
    ...
    /// ```rust,ignore
    ///
    /// let client = mso_graphql_client::Client::new_devnet();
    ...
    pub async fn object(&self, address: Address, version: Option<u64>) -> Result<Option<Object>> {
        ...
        if let Some(object) = response.data {
            ...
            let object = bcs
                .map(|b| bcs::from_bytes::<mso_types::Object>(&b))
                .transpose()?;
    ...
    pub async fn resolve_msons_to_address(&self, domain: &str) -> Result<Option<Address>> {
        ...
        Ok(response
            .data
            .and_then(|d| d.resolve_msons_address)
            .map(|a| a.address))
    }

    /// Get the default MySocialns domain name for the provided address.
    pub async fn default_msons_name(&self, address: Address) -> Result<Option<String>> {
        ...
        Ok(response
            ...
            .and_then(|a| a.default_msons_name))
    }
}
...
mod tests {
    ...
    use mso_types::Ed25519PublicKey;
    use mso_types::TypeTag;
    ...
    async fn test_coin_metadata_query() {
        let client = test_client();
        let cm = client.coin_metadata("0x2::mso::MSO").await;
        assert!(
            cm.is_ok(),
    ...
    async fn test_total_supply() {
        let client = test_client();
        let ts = client.total_supply("0x2::mso::MSO").await;
        assert!(
            ts.is_ok(),
    ...
    async fn test_package_by_name() {
        let client = Client::new_testnet();
        let package = client.package_by_name("mso@mso").await;
